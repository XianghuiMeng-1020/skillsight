...
export const translations: Translations = {
  ...
  'dashboard.graphConnectionHelp': { zh: '悬停职位时，连线表示该岗位所需技能与你的档案的对应关系：与后端返回的「差距技能」同名的技能显示为差距线；其余已掌握技能显示为已满足线。就绪度来自岗位技能要求的加权匹配。', 'zh-TW': '懸停職位時，連線表示該職位所需技能與你檔案的對應：與後端「差距技能」同名的顯示為差距線；其餘已掌握技能為已滿足線。就緒度來自職位技能要求的加權匹配。', en: 'When you hover a role, lines map your skills to that role: dotted = gap (skill name matches the role’s top gap list from the server); solid = you have that skill (level > 0) and it counts toward the link. Readiness is a weighted match against role requirements.' },
